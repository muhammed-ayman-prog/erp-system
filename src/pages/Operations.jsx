  ...
  useState([]);
    ...
    const handleTransfer = async () => {
      ...
      try {
        ...
        await logAction({
  action: LOG_ACTIONS.TRANSFER_STOCK,
  module: LOG_MODULES.INVENTORY,
  ...
  branchId: fromBranch,
  branchName: getBranchName(fromBranch),

  targetId: productId,
  targetName: productMap[productId] || productId,

  details: {
    fromBranch,
    fromBranchName: getBranchName(fromBranch),

    toBranch,
    toBranchName: getBranchName(toBranch),
  ...
  setActiveAction(null);
    ...
    const getLogBranchData = (branchId) => ({
    branchId,
    ...
    const handleUndo = async () => {
      ...
      // 🔥 ADD HERE
      if (oldData.reversed) {
        toast.error(t("operations.alreadyReversed"));
        return;
      }
      ...
      try {
        if (oldData.actionType === "transfer") {
          ...
          const fromRef = doc(db, "inventory", `${oldData.fromBranch}_${oldData.productId}`);
          const toRef = doc(db, "inventory", `${oldData.toBranch}_${oldData.productId}`);
          ...
          await runTransaction(db, async (t) => {
            ...
            if (toQty - amount < 0) {
              throw new Error(
  t("operations.invalidInventoryState")
);
            }
  ...
  t("operations.invalidInventoryState")
          ...
          await logAction({
  action: LOG_ACTIONS.UNDO_ADJUSTMENT,
  module: LOG_MODULES.INVENTORY,
  entityType: "product",
  ...
  targetId: oldData.productId,

  targetName:
    productMap[oldData.productId] ||
    oldData.productId,

  ...getLogBranchData(oldData.branchId),

  details: {
    adjustmentId: oldData.id,
    qty: oldData.qty
  }
});
        }
        ...
        await logAction({
    action: LOG_ACTIONS.UNDO_TRANSFER,
    module: LOG_MODULES.INVENTORY,
    entityType: "product",
    ...
    targetName:
      productMap[oldData.productId] ||
      oldData.productId,
    branchId: oldData.fromBranch,
    branchName: getBranchName(oldData.fromBranch),

    details: {
      transferId: oldData.id,

      fromBranch: oldData.fromBranch,
      fromBranchName: getBranchName(oldData.fromBranch),

      toBranch: oldData.toBranch,
      toBranchName: getBranchName(oldData.toBranch),
    ...
    const handleAdjust = async () => {
      ...
      try {
        ...
        await logAction({
    action: LOG_ACTIONS.ADJUST_STOCK,
    module: LOG_MODULES.INVENTORY,
    entityType: "product",
    ...
    targetId: adjustProduct,
    targetName:
      productMap[adjustProduct] ||
      adjustProduct,

    branchId: adjustBranch,
    branchName: getBranchName(adjustBranch),
    ...
    const handleCreateProduct = async () => {
      ...
      if (
        ...
        toast.error(t("operations.fillRequiredFields"));
  ...
  if (existingProduct) {
    ...
    toast.error(t("operations.productAlreadyExists"));
    ...
    after: {
      ...
      type: newProductType
        ...
        toast.success(t("operations.productCreated"));
  ...
  t("operations.confirmArchive", {
    ...
    try {
      ...
      await logAction({
    action: LOG_ACTIONS.ARCHIVE_PRODUCT,
    module: LOG_MODULES.PRODUCTS,
  ...
  setProductLogs(prev => [
    ...
    ...prev,
    {
      ...
      toast.success(t("operations.productArchived"));
  ...
  const handleRestoreProduct = async (product) => {
    ...
    try {
      ...
      await logAction({
    action: LOG_ACTIONS.RESTORE_PRODUCT,
    module: LOG_MODULES.PRODUCTS,
    ...
    ...prev
      ...
      toast.success(t("operations.productRestored"));
...
if (
  ...
  updatedData.oilCategory =
    null;

  updatedData.pricingTier =
    null;

  updatedData.muskType =
    null;
}

if (updatedData.type === "oil") {

  updatedData.subCategory = null;

  if (updatedData.oilCategory === "French") {

    updatedData.category = "French";

  } else if (
    updatedData.oilCategory === "Oriental"
  ) {

    updatedData.category =
      `Oriental-${updatedData.pricingTier}`;

  } else if (
    updatedData.oilCategory === "Musk"
  ) {

    updatedData.category = "Musk";
  }
...
const beforeData = {
  ...
  oilCategory: editingProduct.oilCategory,

  pricingTier: editingProduct.pricingTier,

  muskType: editingProduct.muskType,
...
const afterData = {
  ...
  oilCategory: updatedData.oilCategory || null,

  pricingTier: updatedData.pricingTier || null,

  muskType: updatedData.muskType || null,
...
await logAction({
  ...
  targetName: editProductData.name,
...
after: changedAfter
  ...
  if (!editData || !selectedActivity) return;

  const oldData = selectedActivity;

  if (Number(oldData.qty) === Number(editData.qty)) {
    toast.error(t("operations.noChanges"));
    return;
  }
  ...
  try {
    ...
    const fromRef = doc(
      db,
      "inventory",
      `${oldData.fromBranch}_${oldData.productId}`
    );

    const toRef = doc(
      db,
      "inventory",
      `${oldData.toBranch}_${oldData.productId}`
    );
          ...
          await runTransaction(db, async (t) => {
            ...
            if (toQty - amount < 0) {
              throw new Error(
  t("operations.invalidReverseTransfer")
);
            }
        ...
        if (oldData.actionType === "adjust") {
          ...
          await runTransaction(db, async (t) => {
            ...
            if (reversed < 0) {
              throw new Error(
  t("operations.invalidReverseOperation")
);
            }
    ...
    const toRef = doc(
      ...
      `${oldData.toBranch}_${oldData.productId}`
          ...
          await runTransaction(db, async (t) => {
            ...
            if (fromQty < amount) throw new Error(
  t("operations.notEnoughStock")
);
  ...
  t("operations.invalidQuantity")
    ...
    setAdjustments(prev =>
      prev.map(item =>
       ...
       await logAction({
  action: LOG_ACTIONS.EDIT_ACTIVITY,
  ...
  details: {
    activityId: oldData.id,
    activityType: oldData.actionType,
  ...
  >
    ...
    >
        ...
        >
          📦 {t("operations.productsManagement")}
        </button>
